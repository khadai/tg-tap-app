...
const Boost: FC<Props> = () => {
    ...
    return (
        <Stack alignItems='center' pt={1.5} sx={{position: 'relative'}}>
            <Typography fontWeight={600} fontSize={13} color='primary.main'>Ваш баланс</Typography>
            ...
            <Stack flexDirection='row' gap={2.5} pt={1} justifyContent='center'>
                <Box bgcolor='#182724' color='primary.main' px='5px' py='2px' borderRadius={2} maxWidth={95}>
                    <Typography fontSize={10} textAlign='center'>Прибыль за тап</Typography>
                    <Stack justifyContent='center' flexDirection='row' gap={0.5}> <DollarIcon/><Typography
                        display='inline' color="common.white" fontWeight={800}
                        fontSize={10}>+{user.oneTapIncome}</Typography></Stack>
                </Box>
                <Box bgcolor='#182724' color='primary.main' px='5px' py='2px' borderRadius={2} maxWidth={95}>
                    <Typography fontSize={10} textAlign='center'>Прибыль в час</Typography>
                    <Stack justifyContent='center' flexDirection='row' gap={0.5}> <DollarIcon/><Typography
                        display='inline' color="common.white" fontWeight={800}
                        fontSize={10}>+{user.oneHourIncome}</Typography></Stack>
                </Box>
            </Stack>
            <Box pt={3}>
                <ButtonBase
                    ...
                    sx={{
                        borderRadius: '50%', backgroundColor: '#1A2725', height: '45px', width: '45px',
                ...
                <Typography fontWeight={800} fontSize={12} color={'common.white'}>BOOST!</Typography>
            </Box>
            <Box sx={{...containerStyle, display: 'flex', mt: '10px', mb: '45px'}}>
                <ZipperIcon/>
                <Box pl={'18px'}>
                    <Typography fontWeight={600} fontSize={10} color={'primary.main'}>Full energy</Typography>
                    <Typography fontWeight={600} fontSize={10} color={'text.secondary'}>6/6 доступно</Typography>
                </Box>
            </Box>
            <Typography fontWeight={800} fontSize={20} color={'common.white'}>Прокачай добычу монет!</Typography>
            {boosts.map(boost =>
                <ButtonBase sx={{...containerStyle, display: 'flex', mt: '10px', alignItems: 'center', height: '65px'}}
            ...
            {selectedBoost && <BoostDialog data={selectedBoost} onClose={() => setSelectedBoost(undefined)}
                                           open={Boolean(selectedBoost)}/>}
        </Stack>
    )
}
