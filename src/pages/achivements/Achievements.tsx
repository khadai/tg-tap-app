...
const Day = (isAvailable: boolean, index: number) => (
    <Box border='0.5px solid #646464' borderRadius='3px' height={27} width={24}>
        <DailyCheckIcon fill={isAvailable ? '#7AFB7F' : '#646464'}/>
        <Typography fontWeight={600} fontSize={5} color='common.white' bgcolor='#646464' borderRadius='2px'
                    pb='1px'>День {index + 1}</Typography>
    </Box>
)
    ...
    return (
        <Box textAlign='center'>
            ...
            <Box sx={{...containerStyle, py: 1, px: 1.5}}>
                <Box display='flex'>
                    <Typography fontWeight={800} fontSize={18} color='common.white' textAlign='start'>ЕЖЕДНЕВНОЕ
                        ВОЗНАГРАЖДЕНИЕ</Typography>
                    <Box>
                        <Typography fontWeight={600} fontSize={8} color='text.secondary'>ДОСТУПНО</Typography>
                        {isAvailable ?
                            <Box display='flex' gap='2px' alignItems='center' justifyContent='center'>
                                <CheckIcon/>
                                <Typography fontWeight={600} fontSize={8} color='#7AFB7F' display='inline'
                                            pt='1px'>СЕЙЧАС</Typography>
                            </Box> : <div><CountDown targetTimestamp={nextAwardTimestamp}/></div>}
                    </Box>
                </Box>
            ...
            {selectedBoost && <AchivementsDialog onClose={() => setSelectedBoost(false)}
                                                  open={Boolean(selectedBoost)} award={award}/>}
        </Box>
    )
}
