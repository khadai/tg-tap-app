...
const FriendListItem: FC<Props> = ({friend}) => {
    ...
    return (
        <Box
            ...
            <Box
                sx={{border: '1.5px #03DABB solid', padding: '2px', width: 'fit-content', borderRadius: '50%'}}
            >
                <Avatar
                    sx={{
            ...
            <Typography fontWeight={600} fontSize={10} color={'common.white'}
                        textAlign='start'>{username}</Typography>
            <Box display={'flex'} gap={0.5} alignItems='center' justifyContent='flex-end' flex={1}>
                <DollarIcon/>
                <Typography
                    fontWeight={800} fontSize={10} color={'common.white'}
                    display='inline'
                >
                    <NumericFormat
                        value={award}
