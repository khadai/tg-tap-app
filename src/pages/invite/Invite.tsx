...
import BoostDialog from "../boost/components/BoostDialog.tsx";
    ...
    return (
        <Box textAlign='center'>
            ...
            <Box display='flex' flexDirection='column' gap='10px'>
                {inviteOffers.map(offer => <OfferListItem key={offer.title} offer={offer}/>)}
            </Box>
            ...
            <Box display='flex' flexDirection='column' gap='5px' maxHeight='250px' sx={{overflowY: 'auto'}}>
                {friendsMock.map(friend => <FriendListItem key={friend.id} friend={friend}/>)}
            </Box>
            ...
            {selectedBoost && <InviteFriendDialog onClose={() => setSelectedBoost(false)}
